...
def dev() -> tuple[tuple[tuple[str, float]], str]:
...
def get_words(response) -> tuple[tuple[str, float]]:
    '''Returns a tuple of tuples that has the values (punctuated_word, start).
    tuple(punctuated_word, start)
    ...
    words = tuple(
        (d['punctuated_word'], round(d['start'], 2))
        for d
